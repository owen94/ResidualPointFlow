...
class InvertibleConv1d(nn.Module):
    ...
    def forward(self, x, logpx=None):
        y = F.conv1d(x, self.weight.view(self.dim, self.dim, 1))
        if logpx is None:
            return y
        else:
            return y, logpx - self._logdetgrad.expand_as(logpx) * x.shape[2]

    def inverse(self, y, logpy=None):
        x = F.conv1d(y, self.weight.inverse().view(self.dim, self.dim, 1))
        if logpy is None:
            return x
        else:
            return x, logpy + self._logdetgrad.expand_as(logpy) * x.shape[2]
