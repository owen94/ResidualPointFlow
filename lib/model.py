...
from lib.resflow1d import pc_resflow, latent_resflow
...
class Encoder(nn.Module):
    def __init__(self, zdim, input_dim=3, use_deterministic_encoder=False):
        ...
        if self.use_deterministic_encoder:
            ...
            self.fc3 = nn.Linear(2048, zdim)
        else:
            # Mapping to [c], cmean
            self.fc1_m = nn.Linear(512, 1024)
            self.fc2_m = nn.Linear(1024, 2048)
            self.fc3_m = nn.Linear(2048, zdim)
            self.fc_bn1_m = nn.BatchNorm1d(1024)
            self.fc_bn2_m = nn.BatchNorm1d(2048)

            # Mapping to [c], cmean
            self.fc1_v = nn.Linear(512, 1024)
            self.fc2_v = nn.Linear(1024, 2048)
            self.fc3_v = nn.Linear(2048, zdim)
            self.fc_bn1_v = nn.BatchNorm1d(1024)
            self.fc_bn2_v = nn.BatchNorm1d(2048)
...
# Model
class PCResFlow(nn.Module):
    def __init__(self, args):
        super(PCResFlow, self).__init__()
        ...
        self.point_rsf = pc_resflow(args)
        self.latent_rsf = latent_resflow(args) if args.use_latent_flow else nn.Sequential()
    ...
    def multi_gpu_wrapper(self, f):
        ...
        self.point_rsf = f(self.point_rsf)
        self.latent_rsf = f(self.latent_rsf)
    ...
    def make_optimizer(self, args):
        def _get_opt_(params):
            ...
            return optimizer
        opt = _get_opt_(list(self.encoder.parameters()) + list(self.point_rsf.parameters())
                        + list(list(self.latent_rsf.parameters())))
        return opt
    ...
    def forward(self, x, step, writer=None):
        ...
        if self.use_latent_flow:
            w, delta_log_pw = self.latent_rsf(z, torch.zeros(batch_size, 1).to(z))
        ...
        y, delta_log_py = self.point_rsf(x, torch.zeros(batch_size, num_points, 1).to(x))
    ...
    def decode(self, z, num_points, truncate_std=None):
        ...
        x = self.point_rsf(y, reverse=True)
        return y, x
    ...
    def sample(self, batch_size, num_points, truncate_std=None, truncate_std_latent=None, gpu=None):
        ...
        if self.use_latent_flow:
            ...
            z = self.latent_rsf(w, None, reverse=True).view(*w.size())
        else:
