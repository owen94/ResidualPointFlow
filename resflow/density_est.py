...
def main():
    ...
    for epoch in range(args.begin_epoch, args.epochs):
        ...
        for bidx in range(100):
            ...
            #
            #print((torch.as_tensor(data[:2048, ])).size())
            inputs = (torch.as_tensor(data[:2048, ]).view(2048, 1, 3)).to(device)
        ...
        if epoch % args.val_freq == 0:
            ...
            with torch.no_grad():
                ...
                _, samples = model.sample(args.val_batchsize, 1, truncate_std=None, gpu=device)
                #samples = model.inverse(torch.randn(args.val_batchsize, args.tr_max_sample_points*3).to(device))
            test_path = os.path.join('density_checkpoints', args.save, 'test_results/')
            ...
            torch.save(model.state_dict(), os.path.join('density_checkpoints', args.save, 'models/model.t7'))
...
if __name__ == '__main__':
    ...
    data = np.load('data/ShapeNetCore.v2.PC15k/02691156/train/ffef991d85e3136a9a6e43b878d5b335.npy')
    print(data.shape)
    ...
    input_size = (args.tr_max_sample_points, 3, 1)
    ...
    model_path = os.path.join('density_checkpoints', args.save, 'models')
